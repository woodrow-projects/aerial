 ...
 *
 * Each channel emits TWO outputs from one pipeline:
 *   1. an HLS rendition set (AAC) written to `hlsDir`  → served by Caddy / CDN
 *   2. one Icecast mount (MP3) pushed to the Icecast container → origin-direct
 */
...
# ── Output 1: HLS rendition set (CDN-cacheable) ───────────────────────────────
output.file.hls(
  playlist="live.m3u8",
...
# ── Output 2: low-latency Icecast mount (origin-direct, never CDN-fronted) ────
output.icecast(
